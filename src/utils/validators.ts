...
export const wishFormSchema = z
  .object({
    ...
    implementationDatetime: z.string().optional(),
  ...
  .superRefine((data, ctx) => {
    if (
      ...
      if (
        !data.participationDatetimeLabel ||
        data.participationDatetimeLabel.trim() === ""
      ) {
        ctx.addIssue({
    ...
    if (
      ...
      if (
        !data.participationNoteLabel ||
        data.participationNoteLabel.trim() === ""
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "備考項目名は必須です",
